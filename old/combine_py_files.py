...
def parse_import_structure(package_dir, file="__main__.py"):
    module_list = []
    dependencies = set()
    with open(package_dir + file, "r", encoding="utf-8") as f:
        for line in f:
            for module in get_modules_from_import_line(line):
                ...
                for m in module.split("."):
                    ...
                    if os.path.isfile(package_dir + path_prefix + ".py"):
                        module_list += parse_import_structure(
                            package_dir, path_prefix + ".py"
                        )
                        dependencies.add(".".join(module_prefix))

                    elif os.path.isfile(package_dir + path_prefix + "/__init__.py"):
                        module_list += parse_import_structure(
                            package_dir, path_prefix + "/__init__.py"
                        )
                        dependencies.add(".".join(module_prefix))
...
def block(file, module_type, module_name, text_block, dependencies):
    ...
    if module_type == "main":
        return main_section_str.format(
        ...
        if dependencies:
            ...
            dependency_text = ""
        return module_section_str.format(
            short_name=short_name,
            name=module_name,
            file=file,
            text=text,
...
def combine_into_one_file(
    ...
    with open(out, "w", encoding="utf-8") as combined:
        ...
        for file_name, dependencies in module_list:
            ...
            with open(package_dir + file_name, "r", encoding="utf-8") as f:
                b = block(
                    file_name,
                    module_type,
                    module_name,
...
if __name__ == "__main__":
    ...
    if os.path.isfile(input_path):
        ...
        start_dir = os.path.join(
            input_path, ""
        )  # puts in proper form with slash at end
